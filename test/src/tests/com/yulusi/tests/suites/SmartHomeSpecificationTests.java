package com.yulusi.tests.suites;

import SmartHomeManagementSystem.DeviceGroup;
...
public final class SmartHomeSpecificationTests {
    ...
    private void testSmartLockStates() {
        harness.run("SmartLock lock/unlock workflow", () -> {
            ...
            Assertions.assertThrows(IllegalArgumentException.class,
                    () -> lock.setState("invalid"),
                    "Invalid states must throw to prevent inconsistent locks");
        });
    }
    ...
    private void testUnknownGroupRejection() {
        harness.run("Unknown groups are rejected", () -> {
            User user = new User("tester-" + branchName);
            Assertions.assertThrows(IllegalArgumentException.class,
                    () -> user.manageGroup("non-existent-" + branchName, "on"),
                    "Hub should throw if a group name is unknown");
        });
    }
